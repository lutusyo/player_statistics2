...
def scouting_reports(request):
    form = ReportFilterForm(request.GET or None)
    queryset = Scouting.objects.all().select_related('squad')
    ...
    return render(request, 'reports_app/2scouting/scouting_reports.html', {'form': form, 'records': queryset})


def export_scouting_excel(request):
    queryset = Scouting.objects.all().select_related('squad')
    df = pd.DataFrame(list(queryset.values('name', 'pos', 'dob', 'squad__name', 'agreement', 'comments', 'date')))
    buffer = io.BytesIO()
...
def export_scouting_pdf(request):
    queryset = Scouting.objects.all().select_related('squad')
    buffer = io.BytesIO()
