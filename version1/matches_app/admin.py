...
@admin.register(Country)                # Country Admin
class CountryAdmin(admin.ModelAdmin):
    list_display = ("name",)
    search_fields = ("name",)
    inlines = [RegionInline]

@admin.register(Region)                 # Region Admin
class RegionAdmin(admin.ModelAdmin):
    list_display = ("name", "country")
    list_filter = ("country",)
    search_fields = ("name", "country__name")

@admin.register(Venue)                  # Venue Admin
...
@admin.register(Competition)                # Competition Admin (NEW)
...
class MatchAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "competition_logo", "competition_name", "venue",
        "get_region", "get_country", "season", "age_group", "group",
    )

    list_filter = (
        "competition__type", "season", "age_group", "group",
        "venue__region__country", "venue__region", )

    search_fields = ("home_team__name", "away_team__name", "venue__name")

    def get_region(self, obj):           # Show region
        return obj.venue.region.name if obj.venue and obj.venue.region else "-"
    get_region.short_description = "Region"

    def get_country(self, obj):         # Show country
        return (
